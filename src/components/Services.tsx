...
import { useInView } from 'framer-motion'
import { useRef } from 'react'
import { Scissors, Sparkles, Hand, Footprints, Smile, ShoppingBag, Droplets, ScissorsLineDashed } from 'lucide-react'
...
const Services = () => {
  ...
  const services = [
    ...
    },
    {
      ...
      description: 'Gommage, enveloppement et modelage pour une peau douce et tonifiée.',
      price: 'À partir de 10.000 FCFA',
      features: ['Gommage corps', 'Enveloppement', 'Modelage', 'Hydratation intense']
    },
    {
  ...
  return (
    <section id="services" className="relative py-24 overflow-hidden bg-pearl lg:py-32">
      ...
      <div className="relative max-w-6xl px-6 mx-auto lg:px-10">
        ...
        {/* Desktop : Grille */}
        <div className="hidden grid-cols-1 gap-6 md:grid lg:grid-cols-4 lg:gap-6">
          {services.map((service, index) => (
            <motion.div
              key={service.title}
              initial={{ opacity: 0, y: 30 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: 0.1 * index, duration: 0.7, ease: [0.25, 0.46, 0.45, 0.94] }}
              whileHover={{ y: -6 }}
              className="relative p-6 transition-all duration-500 bg-white shadow-sm group rounded-2xl hover:shadow-xl"
            >
              {/* Dégradé au hover */}
              <div className="absolute inset-0 transition-opacity duration-500 opacity-0 rounded-2xl bg-gradient-to-br from-prune/5 to-rose/5 group-hover:opacity-100" />
              ...
              <div className="relative z-10">
                {/* Icône violette */}
                <div className="flex items-center justify-center w-12 h-12 mb-5 transition-transform duration-500 shadow-lg bg-gradient-to-br from-prune to-purple-600 rounded-2xl group-hover:scale-110">
                  <service.icon className="w-5 h-5 text-white" />
                </div>
                ...
                <p className="mb-5 text-xs font-light leading-relaxed transition-colors duration-500 text-elegant/40 group-hover:text-elegant/60">
                  {service.description}
                </p>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-black/5">
                  <span className="text-xs font-medium text-prune">
                    {service.price}
                  </span>
                  <a
                    href="#booking"
          ...
          >
            {services.map((service, index) => (
              <motion.div
                ...
                animate={isInView ? { opacity: 1, y: 0 } : {}}
                transition={{ delay: 0.1 * index, duration: 0.6 }}
                className="relative flex-shrink-0 p-5 transition-all duration-500 bg-white shadow-sm snap-center group rounded-2xl hover:shadow-lg w-[260px]"
              >
                {/* Dégradé au hover */}
                <div className="absolute inset-0 transition-opacity duration-500 opacity-0 rounded-2xl bg-gradient-to-br from-prune/5 to-rose/5 group-hover:opacity-100" />
                ...
                <div className="relative z-10">
                  {/* Icône violette */}
                  <div className="flex items-center justify-center mb-4 transition-transform duration-500 shadow-lg w-11 h-11 rounded-xl bg-gradient-to-br from-prune to-purple-600 group-hover:scale-110">
                    <service.icon className="w-5 h-5 text-white" />
                  </div>
                  
                  {/* Numéro */}
                  <span className="block mb-2 text-xs font-light tracking-[0.3em] text-elegant/10 uppercase">
                    {String(index + 1).padStart(2, '0')}
                  </span>
                  
                  <h3 className="mb-1.5 text-base font-medium font-playfair text-elegant">
                    {service.title}
                  </h3>
                  
                  <p className="mb-4 text-xs font-light leading-relaxed text-elegant/40">
                    {service.description}
                  </p>
